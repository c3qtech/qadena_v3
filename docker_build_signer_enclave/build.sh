...
docker buildx build \
  ...
  -f docker_build_signer_enclave/Dockerfile \
  --progress=plain \
  --output type=local,dest=./docker_build_signer_enclave/output \
  .
...
enclave_path="$(pwd)/cmd/signer_enclave"
cp docker_build_signer_enclave/output/signer_enclave $enclave_path
#cp docker_build_enclave/output/qadenad_enclave.unsigned $enclave_path
cp docker_build_signer_enclave/output/reproducible_build_signer_id.txt $enclave_path
cp docker_build_signer_enclave/output/reproducible_build_unique_id.txt $enclave_path
