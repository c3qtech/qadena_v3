...
// RE-SHARE, NOT REKEY: no pubK field.  The handler inherits pubK from the stored row, so this
// message cannot change what the pubKID means -- only who holds shares of it.  shares is the FULL
// new owner set with fresh shares for every owner (old and new): shares from two different Split
// calls combine to garbage, so partial distribution is never valid.
type MsgPioneerUpdatePublicKey struct {
	Creator  string   `protobuf:"bytes,1,opt,name=creator,proto3" json:"creator,omitempty"`
	PubKID   string   `protobuf:"bytes,2,opt,name=pubKID,proto3" json:"pubKID,omitempty"`
	PubKType string   `protobuf:"bytes,3,opt,name=pubKType,proto3" json:"pubKType,omitempty"`
	Shares   []*Share `protobuf:"bytes,4,rep,name=shares,proto3" json:"shares,omitempty"`
	// DER ECDSA signature by the interval private key over
	// sha256("qadena-reshare|" + creator + "|" + pubKID + "|" + pubKType + "|" + sharesJSON):
	// proof the sender HOLDS the key it is re-sharing, verified against the stored row's pubK.
	PossessionSig []byte `protobuf:"bytes,5,opt,name=possessionSig,proto3" json:"possessionSig,omitempty"`
	RemoteReport  []byte `protobuf:"bytes,6,opt,name=remoteReport,proto3" json:"remoteReport,omitempty"`
}

func (m *MsgPioneerUpdatePublicKey) Reset()         { *m = MsgPioneerUpdatePublicKey{} }
func (m *MsgPioneerUpdatePublicKey) String() string { return proto.CompactTextString(m) }
func (*MsgPioneerUpdatePublicKey) ProtoMessage()    {}
func (*MsgPioneerUpdatePublicKey) Descriptor() ([]byte, []int) {
	return fileDescriptor_c730eedddf2fae3b, []int{42}
}
func (m *MsgPioneerUpdatePublicKey) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *MsgPioneerUpdatePublicKey) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_MsgPioneerUpdatePublicKey.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *MsgPioneerUpdatePublicKey) XXX_Merge(src proto.Message) {
	xxx_messageInfo_MsgPioneerUpdatePublicKey.Merge(m, src)
}
func (m *MsgPioneerUpdatePublicKey) XXX_Size() int {
	return m.Size()
}
func (m *MsgPioneerUpdatePublicKey) XXX_DiscardUnknown() {
	xxx_messageInfo_MsgPioneerUpdatePublicKey.DiscardUnknown(m)
}

var xxx_messageInfo_MsgPioneerUpdatePublicKey proto.InternalMessageInfo

func (m *MsgPioneerUpdatePublicKey) GetCreator() string {
	if m != nil {
		return m.Creator
	}
	return ""
...
func (m *MsgPioneerUpdatePublicKey) GetPubKID() string {
	if m != nil {
		return m.PubKID
	}
	return ""
...
func (m *MsgPioneerUpdatePublicKey) GetPubKType() string {
	if m != nil {
		return m.PubKType
	}
	return ""
...
func (m *MsgPioneerUpdatePublicKey) GetShares() []*Share {
	if m != nil {
		return m.Shares
	}
	return nil
...
func (m *MsgPioneerUpdatePublicKey) GetPossessionSig() []byte {
	if m != nil {
		return m.PossessionSig
	}
	return nil
}

func (m *MsgPioneerUpdatePublicKey) GetRemoteReport() []byte {
	if m != nil {
		return m.RemoteReport
	}
	return nil
...
type MsgPioneerUpdatePublicKeyResponse struct {
}

func (m *MsgPioneerUpdatePublicKeyResponse) Reset()         { *m = MsgPioneerUpdatePublicKeyResponse{} }
func (m *MsgPioneerUpdatePublicKeyResponse) String() string { return proto.CompactTextString(m) }
func (*MsgPioneerUpdatePublicKeyResponse) ProtoMessage()    {}
func (*MsgPioneerUpdatePublicKeyResponse) Descriptor() ([]byte, []int) {
	return fileDescriptor_c730eedddf2fae3b, []int{43}
}
func (m *MsgPioneerUpdatePublicKeyResponse) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *MsgPioneerUpdatePublicKeyResponse) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_MsgPioneerUpdatePublicKeyResponse.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *MsgPioneerUpdatePublicKeyResponse) XXX_Merge(src proto.Message) {
	xxx_messageInfo_MsgPioneerUpdatePublicKeyResponse.Merge(m, src)
}
func (m *MsgPioneerUpdatePublicKeyResponse) XXX_Size() int {
	return m.Size()
}
func (m *MsgPioneerUpdatePublicKeyResponse) XXX_DiscardUnknown() {
	xxx_messageInfo_MsgPioneerUpdatePublicKeyResponse.DiscardUnknown(m)
}

var xxx_messageInfo_MsgPioneerUpdatePublicKeyResponse proto.InternalMessageInfo
...
type MsgSignRecoverPrivateKey struct {
	...
	// WHO THE GUARDIAN ACTUALLY VERIFIED.
	//
	// An institutional guardian's app server resolves identity -> wallet entirely on its own side
	// (an authenticated email, then a NameBinding lookup) and used to put only the encrypted
	// DESTINATION wallet id in this message.  So the enclave could not check that the human who
	// authenticated had anything to do with the wallet being signed: if the server resolved the
	// wrong user, every enclave check still passed, and the signature landed on a stranger's
	// recovery -- consuming the guardian's one signature there and stranding the real user, since
	// the per-record dedup allows a guardian to sign only once.
	//
	// These carry the identity hash the guardian verified, so the enclave can resolve it to a wallet
	// through its OWN indexes (getCredentialByHash -> getProtectSubWalletIDByOriginalWalletID) and
	// compare.  The enclave never takes a wallet id from the guardian -- same discipline as the
	// client-supplied hash that is checked against an enclave-derived value elsewhere.
	//
	// Payload is an EncryptableString whose Value is the hex of common.CreateCredentialHash.
	// OPTIONAL on the wire: whether it is required depends on the guardian's class and on
	// Params.sign_recover_key_guardian_assertion_mode, neither of which ValidateBasic can see.
	EncGuardianCredentialHashVShare  []byte          `protobuf:"bytes,5,opt,name=encGuardianCredentialHashVShare,proto3" json:"encGuardianCredentialHashVShare,omitempty"`
	GuardianCredentialHashVShareBind *VShareBindData `protobuf:"bytes,6,opt,name=guardianCredentialHashVShareBind,proto3" json:"guardianCredentialHashVShareBind,omitempty"`
}
...
func (m *MsgSignRecoverPrivateKey) GetEncGuardianCredentialHashVShare() []byte {
	if m != nil {
		return m.EncGuardianCredentialHashVShare
	}
	return nil
}

func (m *MsgSignRecoverPrivateKey) GetGuardianCredentialHashVShareBind() *VShareBindData {
	if m != nil {
		return m.GuardianCredentialHashVShareBind
	}
	return nil
}
...
func (*MsgCreateBulkCredentials) Descriptor() ([]byte, []int) {
	return fileDescriptor_c730eedddf2fae3b, []int{60}
}
...
func (*MsgCreateBulkCredentialsResponse) Descriptor() ([]byte, []int) {
	return fileDescriptor_c730eedddf2fae3b, []int{61}
}
...
func init() {
	...
	proto.RegisterType((*MsgPioneerUpdatePublicKey)(nil), "qadena.qadena.MsgPioneerUpdatePublicKey")
	proto.RegisterType((*MsgPioneerUpdatePublicKeyResponse)(nil), "qadena.qadena.MsgPioneerUpdatePublicKeyResponse")
...
var fileDescriptor_c730eedddf2fae3b = []byte{
	...
	,
}
...
type MsgClient interface {
	...
	// Re-shares an EXISTING key's secret shares to a grown owner set.  Add rejects an existing
	// pubKID and Update rejects a missing one -- together the existence check stays the ACL.  The
	// handler additionally requires a PROPER SUPERSET of the current owners and a possession
	// signature verifiable against the STORED row's pubK; the message carries no pubK field at all,
	// so it structurally cannot re-key.
	PioneerUpdatePublicKey(ctx context.Context, in *MsgPioneerUpdatePublicKey, opts ...grpc.CallOption) (*MsgPioneerUpdatePublicKeyResponse, error)
...
func (c *msgClient) PioneerUpdatePublicKey(ctx context.Context, in *MsgPioneerUpdatePublicKey, opts ...grpc.CallOption) (*MsgPioneerUpdatePublicKeyResponse, error) {
	out := new(MsgPioneerUpdatePublicKeyResponse)
	err := c.cc.Invoke(ctx, "/qadena.qadena.Msg/PioneerUpdatePublicKey", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type MsgServer interface {
	...
	// signature verifiable against the STORED row's pubK; the message carries no pubK field at all,
	// so it structurally cannot re-key.
	PioneerUpdatePublicKey(context.Context, *MsgPioneerUpdatePublicKey) (*MsgPioneerUpdatePublicKeyResponse, error)
...
func (*UnimplementedMsgServer) PioneerUpdatePublicKey(ctx context.Context, req *MsgPioneerUpdatePublicKey) (*MsgPioneerUpdatePublicKeyResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method PioneerUpdatePublicKey not implemented")
}
...
func _Msg_PioneerUpdatePublicKey_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(MsgPioneerUpdatePublicKey)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MsgServer).PioneerUpdatePublicKey(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/qadena.qadena.Msg/PioneerUpdatePublicKey",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MsgServer).PioneerUpdatePublicKey(ctx, req.(*MsgPioneerUpdatePublicKey))
	}
	return interceptor(ctx, in, info, handler)
}
...
var _Msg_serviceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "PioneerUpdatePublicKey",
			Handler:    _Msg_PioneerUpdatePublicKey_Handler,
		},
		{
...
func (m *MsgPioneerUpdatePublicKey) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *MsgPioneerUpdatePublicKey) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *MsgPioneerUpdatePublicKey) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if len(m.PossessionSig) > 0 {
		i -= len(m.PossessionSig)
		copy(dAtA[i:], m.PossessionSig)
		i = encodeVarintTx(dAtA, i, uint64(len(m.PossessionSig)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.Shares) > 0 {
...
func (m *MsgPioneerUpdatePublicKeyResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *MsgPioneerUpdatePublicKeyResponse) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *MsgPioneerUpdatePublicKeyResponse) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *MsgSignRecoverPrivateKey) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if m.GuardianCredentialHashVShareBind != nil {
		{
			size, err := m.GuardianCredentialHashVShareBind.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
	...
	if len(m.EncGuardianCredentialHashVShare) > 0 {
		i -= len(m.EncGuardianCredentialHashVShare)
		copy(dAtA[i:], m.EncGuardianCredentialHashVShare)
		i = encodeVarintTx(dAtA, i, uint64(len(m.EncGuardianCredentialHashVShare)))
		i--
		dAtA[i] = 0x2a
	}
...
func (m *MsgPioneerUpdatePublicKey) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *MsgPioneerUpdatePublicKeyResponse) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *MsgSignRecoverPrivateKey) Size() (n int) {
	...
	l = len(m.EncGuardianCredentialHashVShare)
	if l > 0 {
		n += 1 + l + sovTx(uint64(l))
	}
	if m.GuardianCredentialHashVShareBind != nil {
		l = m.GuardianCredentialHashVShareBind.Size()
		n += 1 + l + sovTx(uint64(l))
	}
	return n
}
...
func (m *MsgPioneerUpdatePublicKey) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: MsgPioneerUpdatePublicKey: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: MsgPioneerUpdatePublicKey: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PossessionSig", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.PossessionSig = append(m.PossessionSig[:0], dAtA[iNdEx:postIndex]...)
			if m.PossessionSig == nil {
				m.PossessionSig = []byte{}
			}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
...
func (m *MsgPioneerUpdatePublicKeyResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: MsgPioneerUpdatePublicKeyResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: MsgPioneerUpdatePublicKeyResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
...
func (m *MsgSignRecoverPrivateKey) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field EncGuardianCredentialHashVShare", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.EncGuardianCredentialHashVShare = append(m.EncGuardianCredentialHashVShare[:0], dAtA[iNdEx:postIndex]...)
			if m.EncGuardianCredentialHashVShare == nil {
				m.EncGuardianCredentialHashVShare = []byte{}
			}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field GuardianCredentialHashVShareBind", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.GuardianCredentialHashVShareBind == nil {
				m.GuardianCredentialHashVShareBind = &VShareBindData{}
			}
			if err := m.GuardianCredentialHashVShareBind.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
