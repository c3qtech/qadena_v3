...
type EncryptablePioneerIDs struct {
	...
	// GENERATION: the chain height at which this node LEARNED this owner set from a live view of
	// the chain.  Not the height the split was established at, and deliberately not taken from the
	// message that carries the owner list -- that message may be history being replayed.
	//
	// The owners record is reconstruction METADATA ABOUT NOW: which pioneers hold shares today and
	// therefore, via getThreshold, what those shares ARE.  It has several writers and no ordering,
	// so the last one won -- and on a replaying node the last writer is the OLDEST block.  That is
	// how a joiner rewound seven keys from five owners to one, read back "threshold 1", and handed
	// a 65-byte Shamir share to ScalarMult.
	//
	// A write now wins only if its generation is >= the stored one.  ">=" not ">": two re-shares
	// can land in the same block and must apply in order.  Absent (0) loses to everything real,
	// which is what makes an unknown chain position safe rather than dangerous.
	//
	// NOT CONSENSUS STATE.  This record lives in the unversioned secrets DB and is hashed into
	// nothing, so two nodes disagreeing about a generation changes no block outcome.
...
func (m *EncryptablePioneerIDs) GetGeneration() int64 {
	if m != nil {
		return m.Generation
	}
	return 0
...
type EncryptableSyncEnclavePage struct {
	...
	// THE SEED'S VIEW OF WHERE EVERY PIONEER LIVES, keyed by pioneerID.  First page only: unlike
	// the owners map this is a handful of entries and does not page.
	//
	// A BOOTSTRAP CRUTCH WITH A DEFINED END, not a second source of truth.  The joiner's mirrored
	// IntervalPublicKeyID rows are authoritative for addresses, but they arrive at REPLAY speed --
	// so a peer that published its address above the joiner's snapshot looks undiallable until
	// catch-up reaches that block, and getSSPrivK cannot ask it for a share.  Seen on the
	// 2026-08-26 bringup: "no address for pioneer pioneer3 -- cannot ask it for a share", which
	// survived only because the key needed one share and two other owners were reachable.  With a
	// threshold of 2 and two owners briefly dark, that is INSUFFICIENT, which backlog 90 says the
	// caller must halt on.
	//
	// Used ONLY as a fallback, ONLY while replaying, and DELETED at the replay->live transition:
	// after that the mirrored rows are current and have the only claim.  It therefore needs no
	// generation guard -- replay never writes to it -- and it cannot diverge the store hash,
	// because it never touches the mirrored store.  Consensus-safe by construction: it decides
	// which peer this node DIALS, and a wrong answer costs a failed fetch, not wrong state -- the
	// share is checked by attestation and the reconstructed key by the derive-to-pubK test.
	PioneerAddresses map[string]string `protobuf:"bytes,6,rep,name=pioneerAddresses,proto3" json:"pioneerAddresses,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// The SEED's height when it served this page, stamped onto every owner record the joiner
	// installs.  The joiner is about to replay history from genesis, and every owner list in that
	// history is older than this -- so one number, taken once from a node that is demonstrably
	// live, immunises the whole map against the replay that follows.  Without it a fresh joiner's
	// records would sit at generation 0 and lose to the first old block that names them.
	SeedHeight int64 `protobuf:"varint,5,opt,name=seedHeight,proto3" json:"seedHeight,omitempty"`
}
...
func (m *EncryptableSyncEnclavePage) GetPioneerAddresses() map[string]string {
	if m != nil {
		return m.PioneerAddresses
	}
	return nil
}

func (m *EncryptableSyncEnclavePage) GetSeedHeight() int64 {
	if m != nil {
		return m.SeedHeight
	}
	return 0
...
func init() {
	...
	proto.RegisterMapType((map[string]string)(nil), "qadena.qadena.EncryptableSyncEnclavePage.PioneerAddressesEntry")
...
var fileDescriptor_ef4cf267f6afd07c = []byte{
	...
	,
}
...
func (m *EncryptablePioneerIDs) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	if m.Generation != 0 {
		i = encodeVarintEncryptable(dAtA, i, uint64(m.Generation))
		i--
		dAtA[i] = 0x10
	}
...
func (m *EncryptableSyncEnclavePage) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.PioneerAddresses) > 0 {
		for k := range m.PioneerAddresses {
			v := m.PioneerAddresses[k]
			baseI := i
			i -= len(v)
			copy(dAtA[i:], v)
			i = encodeVarintEncryptable(dAtA, i, uint64(len(v)))
	...
	if m.SeedHeight != 0 {
		i = encodeVarintEncryptable(dAtA, i, uint64(m.SeedHeight))
		i--
		dAtA[i] = 0x28
	}
	if m.Done {
...
func (m *EncryptablePioneerIDs) Size() (n int) {
	...
	if m.Generation != 0 {
		n += 1 + sovEncryptable(uint64(m.Generation))
	}
	return n
}
...
func (m *EncryptableSyncEnclavePage) Size() (n int) {
	...
	if m.SeedHeight != 0 {
		n += 1 + sovEncryptable(uint64(m.SeedHeight))
	}
	if len(m.PioneerAddresses) > 0 {
		for k, v := range m.PioneerAddresses {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + sovEncryptable(uint64(len(k))) + 1 + len(v) + sovEncryptable(uint64(len(v)))
			n += mapEntrySize + 1 + sovEncryptable(uint64(mapEntrySize))
...
func (m *EncryptableSyncEnclavePage) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SeedHeight", wireType)
			}
			m.SeedHeight = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.SeedHeight |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PioneerAddresses", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.PioneerAddresses == nil {
				m.PioneerAddresses = make(map[string]string)
			}
			var mapkey string
			var mapvalue string
			for iNdEx < postIndex {
			...
			m.PioneerAddresses[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipEncryptable(dAtA[iNdEx:])
