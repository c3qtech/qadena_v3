...
110. `ERR_EXIT` + `PIPE_FAIL` turns a no-match grep into a SILENT script death

     Found 2026-08-24 in rolling_upgrade.sh, on the first real dry-run of `--build-from`. The
     script stopped dead after printing `running: version=1.1.14`, with no message and exit 1.

     The cause is a one-liner that looks completely ordinary:

         st=$(identity_status "$SRC_UNIQ")

     `identity_status` ends in `... | grep -oE '"status":"[a-z]+"' | head -1 | cut -d'"' -f4`.
     For a measurement the chain has never seen, grep matches nothing and exits 1; `setopt
     PIPE_FAIL` promotes that to the pipeline's status; `setopt ERR_EXIT` then kills the script
     at the assignment. No trap, no message, nothing on stderr.

     WHAT MAKES IT NASTY, not merely annoying:
       - An UNREGISTERED measurement is the NORMAL state at the start of every roll. This was not
         an edge case; it was the main path.
       - The `die` written specifically to explain the situation is on the NEXT line and is
         therefore unreachable. The better the error handling reads, the more certainly it is
         dead code.
       - It only bites inside an ASSIGNMENT. The same helper inside `[[ ... ]]` or `$( )` in an
         argument is fine, so the construct appears to work everywhere else in the script.

     Fixed there by appending `|| true` to 19 assignments (15 new, 4 pre-existing in the
     governance block, where the same defect had made three existing `die` messages unreachable).

     STILL WORTH DOING: this is a property of every script in the repo that sets both options and
     assigns from a pipeline containing grep/jq/python. testscripts/ and scripts/ have not been
     swept for it. A sweep is mechanical -- find `setopt` lines carrying both options, then look
     for `X=$(... | grep ...)` without a guard. Worth doing before another one of these is
     discovered by a deploy stopping halfway with no explanation.

111. `docs/static/openapi.yml` is tracked but REGENERATED by every build

     Found 2026-08-24 while adding a dirty-tree guard to rolling_upgrade.sh. The primary's
     checkout showed `M docs/static/openapi.yml` with a one-line diff; the build rewrites it.

     So the tree is dirty after every single build, which means any guard that insists on a clean
     tree -- and any operator running `git status` to decide whether a checkout is trustworthy --
     gets a false positive every time. The signal that should mean "someone edited this checkout"
     instead means "someone built here", which is the normal case.

     Either gitignore it, or commit the generated output as part of the build's contract. Right
     now it is the worst of both: tracked, so it shows up as a change, and generated, so the
