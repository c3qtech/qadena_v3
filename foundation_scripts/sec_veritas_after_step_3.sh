...
echo "        --armor-prompt \\"
echo "        --node-host <the chain THIS deployment runs on> --grpc-port 9090 \\"
...
echo ""
echo "  --node-host SETS WHICH CHAIN THE APP-SERVER TALKS TO.  Everything else here comes out"
echo "  of the ceremony; the endpoint does not, so without it the template keeps whatever the"
echo "  source carried -- a DIFFERENT chain's address.  The result authenticates with this"
echo "  deployment's keys against that other chain and fails every lookup, naming nothing."
echo "  It writes QADENA_PIONEER_IP and QADENA_PUBLIC_HOST, which are normally the same host;"
echo "  they differ only where the server reaches the chain privately and phones cannot."
echo "  This run's node is ${QADENA_NODE:-<unset>}."
echo ""
