...
FROM="bootstrap"
REBUILD=0
# ONE PATH, NOT A DIRECTORY PLUS A NAME.  --stack and --env-file had to agree, and a run that
# changed one and not the other patched a file in the wrong stack or a name that did not exist
# there.  The env file's own location IS the stack -- everything else (compose.yml, the Makefile,
# .env) sits beside it -- so take the full path and derive the rest.
ENV_FILE="$HOME/test/follow-the-money/stacks/veritas/env-staging-no-aws"
PREFIX="sec"
