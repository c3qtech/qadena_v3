...
if [ -z "$proposal_id" ] || [ -z "$amount" ]; then
    echo "Usage: $0 <proposal_id> <amount>"
    exit 1
fi


echo "-------------------------"
echo "Deposit from treasury"
echo "-------------------------"
echo "Depositing $amount to proposal $proposal_id from treasury"
result=$(qadenad_alias tx gov deposit $proposal_id $amount --from treasury -y --output json --gas-prices $minimum_gas_prices --gas auto --gas-adjustment $gas_adjustment)
echo "Result: $result"
# get tx hash
