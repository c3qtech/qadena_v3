...
run_init() {
    ...
    # THE DOCKER OUTPUT DIRECTORIES TOO, for the same reason and with a nastier failure.  A previous
    # run under sudo leaves these owned by root; the reproducible build, now running as the user,
    # cannot export into them and buildkit fails with
    #
    #     error from receiver: failed to create .tmp.NNNNNNN: permission denied
    #
    # twenty minutes in.  They are gitignored build outputs, regenerated by every build, so removing
    # them costs nothing but a cache miss.
    local d
