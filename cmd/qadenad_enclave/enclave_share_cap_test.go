...
// The caller's slice is the live result of getAddressPublishedPioneers; selecting must not reorder it.
...
func TestPlanSSRotationSnapshotsEverythingTheRotationNeeds(t *testing.T) {
	...
	// Registered but never published an address: getAddressPublishedPioneers excludes it, so the plan
