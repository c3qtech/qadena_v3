...
# FAIL BEFORE THE BUILD, NOT AFTER IT.  The keyring migration runs after install.sh -- it needs a
# qadenad, and it must not delete keyring-test before setPubKAndPubKID.sh has read it -- so its own
# "no passphrase" guard fires at the very END of a ~25 minute build.  Refusing here instead costs
# nothing and tells the operator while it is still true that nothing has been changed.
if grep -qE '^[[:space:]]*keyring-backend:[[:space:]]*"?file"?' "$config_src" 2>/dev/null \
   && [[ -z "$keyring_passfile" ]]; then
    echo "   INIT FAILED: $config_src asks for keyring-backend: file, which needs a passphrase."
    echo "     --keyring-passfile <file>   (first line is the passphrase for the node keyring)"
    echo ""
    echo "   ignite creates the keys in keyring-test whatever the config says, so they are migrated"
    echo "   after the build and the unencrypted copy removed.  Without the passphrase this build"
    echo "   would run to completion and then stop, ~25 minutes from now.  Nothing has been changed."
    exit 1
fi
...
if ignite chain init --home $QADENAHOME ; then
    ...
    # accounts in keyring-test regardless.  A build
    ...
    # THE KEYRING MIGRATION USED TO BE HERE, and both reasons it moved are worth keeping:
    #
    #   1. $QADENAHOME/bin/qadenad does not exist yet -- install.sh creates it, ~250 lines below --
    #      so migrate_keyring.sh could not read the keyring at all and reported "no keys found"
    #      against a keyring that was full.
    #   2. setPubKAndPubKID.sh below pins --keyring-backend test.  Migrating first DELETES
    #      keyring-test, so the devnet path would have lost the keyring it reads.  Only the launch
    #      path (--mainnet-source) skips that script, which is why this was not seen sooner.
    #
    # It now runs after install.sh.  See "migrate the node keyring" further down.
