...
if [[ $REAL_ENCLAVE == 1 ]] ; then
    ...
    ego-go build -o $enclave_path/qadena_ekyc -mod readonly qadena/cmd/qadena_ekyc || exit 1
    cd $enclave_path
    ...
    go build -o $user_go_path/qadena_ekyc -mod readonly qadena/cmd/qadena_ekyc || exit 1
fi
