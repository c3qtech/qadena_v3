...
if [[ $build_reproducible == 1 ]] ; then
  if [[ "$DOCKER_BUILD" = "1" ]]; then
    ...
    ego-go version
    echo "Build..."
    ego-go build -buildvcs=false -ldflags="-s -w" -trimpath -o $enclave_path/qadenad_enclave -mod=vendor qadena/cmd/qadenad_enclave || exit 1
    cd $enclave_path
#    cp qadenad_enclave qadenad_enclave.unsigned
  ...
  echo "-------------------"
  cd $qadenabuild
  go build -trimpath -o $enclave_path/qadenad_enclave -mod=vendor qadena/cmd/qadenad_enclave || exit 1
